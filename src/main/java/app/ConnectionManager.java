...
public class ConnectionManager {
        ...
        if (activeConnection==null) {
            try {
                ...
                try {
                    ...
                    //запустити тут потік для перевірки чи доступна 1 база
    ...
    private void checkMasterDB(){
        try {
            Connection testMasterDB = DriverManager.getConnection(connectionPool.getMasterUrl(),connectionPool.getUser(),connectionPool.getPassword());
            boolean available = testMasterDB.isValid(1);
            if (available==true) {
                activeConnection = null;
                getLog.info("MasterDB is available");
            }
        } catch (SQLException throwables) {
            getLog.error("MasterDB is not available ",throwables);
            throwables.printStackTrace();
        }
    }
}
